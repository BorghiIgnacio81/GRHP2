...
class ValidacionesTest(TestCase):
    def setUp(self):
        # crear registros relacionados necesarios
        ...
        # plan: solo trabaja lunes a viernes
    ...
    def test_rechaza_si_todo_feriado(self):
        ...
        lic = Solicitud_licencia(
            idempleado=self.empleado,
            fecha_desde=feriado_dia,
            fecha_hasta=feriado_dia,
            id_estado=Estado_lic_vac.objects.get(estado='Pendiente'),
            id_licencia=Tipo_licencia.objects.first(),
        )
        with self.assertRaises(ValidacionError) as cm:
            validar_solicitud_licencia(lic)
        self.assertIn('contiene feriado', str(cm.exception))
    ...
    def test_warn_si_parcial_feriado(self):
        ...
        self.assertTrue(ok)
        self.assertIn('contiene feriado', warnings)

    def test_rechaza_si_no_laboral_total(self):
        # Crear un plan que no trabaja en las dos fechas
        ...
        emp2 = Empleado.objects.create(
            ...
            dni='11111111',
            fecha_nac=date(1990, 1, 1),
            id_nacionalidad=Nacionalidad.objects.first(),
            id_civil=EstadoCivil.objects.first(),
            id_sexo=Sexo.objects.first(),
            id_localidad=Localidad.objects.first(),
            dr_personal='',
            telefono='',
            cuil='20-87654321-9',
        )
    ...
    def test_rechaza_solapamiento_licencia(self):
        ...
        s1 = Solicitud_licencia.objects.create(
            idempleado=self.empleado,
            fecha_desde=d1,
            fecha_hasta=d2,
            id_estado=estado_acept,
            id_licencia=tipo,
        )
        s2 = Solicitud_licencia(
            idempleado=self.empleado,
            fecha_desde=d1,
            fecha_hasta=d1,
            id_estado=Estado_lic_vac.objects.get(estado='Pendiente'),
